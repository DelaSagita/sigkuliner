	...
	<script>
		...
		function initialize(){
		  ...
		  var infoWindow = new google.maps.InfoWindow;
			...
			<?php
				if (isset($_GET['filter']) && !empty($_GET['rangeHarga'])) {
					...
					$query = mysqli_query($con,"select * from warung WHERE $rangeHarga");
			    while ($data = mysqli_fetch_array($query))
			    {
					...
					$query = mysqli_query($con,"select * from warung");
			    while ($data = mysqli_fetch_array($query))
			    {
			        $nama = $data['nama_warung'];
			        $alamat = $data['alamat'];
		  ...
		  function bindInfoWindow(lat, lng, marker, map, infoWindow, inama, ialamat, igambar, iharga, ijamBuka, ijamTutup, ideskripsi) {
		      ...
		      document.getElementById("alamatW").innerHTML="<i class='fa fa-map-marker fa-2x' aria-hidden='true'></i><h class='tab'>Alamat : "+ialamat+"</h>";
		      document.getElementById("gambarW").setAttribute('src', 'gambar/'+igambar );
		      document.getElementById("gambarW").setAttribute('style', "border: 1px solid black; width:100px");
		      document.getElementById("gambarShow").setAttribute('src', 'gambar/'+igambar);
		      document.getElementById("hargaW").innerHTML="<i class='fa fa-usd fa-2x' aria-hidden='true'></i><h class='tab'>Harga : Rp. "+iharga+"</h>";
		      document.getElementById("jam").innerHTML="<i class='fa fa-clock-o fa-2x' aria-hidden='true'></i><h class='tab'>Jam Buka : "+ijamBuka+" - "+ijamTutup+" WIB</h>";
		      document.getElementById("deskripsiW").innerHTML="<i class='fa fa-info-circle fa-2x' aria-hidden='true'></i><h class='tab'>Keterangan :<br>"+ideskripsi+"</h>";
		    });
		  }
		}
		...
		$(document).ready(function() {
			...
			$("#cari").click(function(){
				dest = ''+latlong+'';
				var defaultLatLng = new google.maps.LatLng(-1.8456323,109.977800);
				...
				<div class="col-md-3">
					...
					<aside>
						...
						<p>
						  ...
						  <input type="text" id="amount" readonly style="border:0; color:#FF007F; font-weight:bold;">
						</p>
						...
						<p><span id="hargaW"></span></p>
						<p><span id="alamatW"></span></p>
