	...
	if (is_numeric($nama_warung)){
		...
		if($file_size > $max_size){
			...
			if(move_uploaded_file($_FILES['upload_gambar']['tmp_name'], $folder.$file_name)){
				...
				$sql = "INSERT INTO `warung` (`nama_warung`, `alamat`, `latitude`, `longitude`, `gambar`, `harga`, `jam_buka`, `jam_tutup`, `deskripsi`) VALUES ('$nama_warung', '$alamat', '$latitude', '$longitude', '$file_name', '$harga', '$jam_buka', '$jam_tutup', '$deskripsi')";
				$query = mysqli_query($con, $sql);
				if ($query) {
					echo '<div id="msg">Berhasil mengupload file '.$file_name.'</div>';
					header("location: http://localhost/sigkuliner/pengaturan-data.php");
				} else {
